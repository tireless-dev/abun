...
- reviewed on 2026-06-25: shared UI bottom-sheet flows now live in the same Compose Navigation graph as top-level screens, with route-backed destinations for task detail, routine detail, create-task, create-routine, start-pomodoro, and complete-pomodoro; task and routine detail routes pass only stable ids and resolve live state inside the nav host
- reviewed on 2026-06-25: `ui/components/TaskRows.kt` now uses a clickable flat card with a status dot only, removing both the prior status label and the explicit detail CTA button text
- reviewed on 2026-06-25: `ui/sheets/TaskActionsSheet.kt` now uses a custom bottom-anchored modal sheet container on desktop so the visible sheet stays pinned to the bottom edge at an 80% screen-height target, opens in read-only presentation with no execution buttons, switches into in-place edit mode from a compact top-right edit affordance, and gates destructive delete through an explicit confirmation dialog
